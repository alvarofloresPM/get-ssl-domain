        ...
        stage('Create domain and SSL CertBot') {
            steps {
                sshagent (credentials: ['4326e3ee-90e1-4e8f-ad31-084a0cbec30d']) {
                    sh "if ! ssh -o StrictHostKeyChecking=no -l root 10.100.8.118 /opt/sslcert/SslScript.sh ${params.IPserver} ${params.SubDomain}.${params.domain} ${params.port} ${params.protocol} ${params.ProcessMaker4} ${params.LocalNetwork} ${params.PortEchoPM4}; then echo 'FAILURE - the server is down or do not exist also the configuration already exist in the server' ; exit 1 ; fi "
                }
            }     
        }
